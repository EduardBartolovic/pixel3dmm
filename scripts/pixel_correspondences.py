...
def main():
    ...
    for vid_name in tqdm(sorted(os.listdir(TRACKING_OUTPUT))):
        vid_name = vid_name[:-27]
        vid_path = os.path.join(TRACKING_OUTPUT, vid_name)

        print(f"\n=== Processing {vid_name} ===")
        tracking_dir = f'{TRACKING_OUTPUT}/{vid_name}_nV1_noPho_uv2000.0_n1000.0'
        if not os.path.isdir(tracking_dir):
            print(f"Video path not found {tracking_dir}")
            continue

        mesh_path = os.path.join(tracking_dir, 'mesh')
        ckpt_path = os.path.join(tracking_dir, 'checkpoint')

        if not os.path.exists(mesh_path) or not os.path.exists(ckpt_path):
            print(f"Skipping {vid_name} — missing mesh or checkpoint folder")
            continue

        out_dir = f'/home/gustav/pixel3dmm/corr_out/{vid_name}'
        os.makedirs(out_dir, exist_ok=True)

        print("Calculating visible faces...")
        vis_data = calculate_visible_faces(ckpt_path, mesh_path, IMAGE_WIDTH, IMAGE_HEIGHT)

        ref_data = vis_data.get(ref_frame)
        if ref_data is None:
            print(f"Skipping {vid_name} — no reference frame {ref_frame}")
            continue
        ...
        for idx, data in vis_data.items():
            ...
            if len(common_faces) == 0:
                print(f"No common_faces for {vid_name}, frame {idx}")
                continue
            correspondences[idx] = {
                **data,
