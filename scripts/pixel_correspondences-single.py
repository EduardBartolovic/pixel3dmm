...
def visible_faces_mask(verts_proj, verts_cam, faces, size, z_bias=1e-6):
    verts_z = -verts_cam[:, 2]

    z_buffer = np.full((size, size), np.inf, dtype=np.float32)
    tri_buffer = np.full((size, size), -1, dtype=np.int32)
    ...
    for tri_id, (i0, i1, i2) in enumerate(faces):
        ...
        max_x = min(int(np.ceil(np.max(pts[:, 0]))), size - 1)
        min_y = max(int(np.floor(np.min(pts[:, 1]))), 0)
        max_y = min(int(np.ceil(np.max(pts[:, 1]))), size - 1)
        ...
        depth_inside = (u[inside] * z_vals[0] + v[inside] * z_vals[1] + w[inside] * z_vals[2])

        # Depth test
        for (x, y), depth in zip(pix_inside.astype(int), depth_inside):
            if depth < z_buffer[y, x] - z_bias:
                z_buffer[y, x] = depth
...
def calculate_visible_faces(ckpt_path, mesh_path, size):
    ...
    for idx in range(N):
        ...
        intr = np.eye(3)
        fl = ckpt['camera']['fl'][0, 0] * size
        intr[0, 0], intr[1, 1] = fl, fl
        intr[0, 2] = ckpt['camera']['pp'][0, 0] * (size / 2 + 0.5) + size / 2 + 0.5
        intr[1, 2] = ckpt['camera']['pp'][0, 1] * (size / 2 + 0.5) + size / 2 + 0.5

        verts_proj, verts_cam = project_points(mesh.vertices, intr, extr)

        valid_mask = (
            (verts_proj[:, 0] >= 0) & (verts_proj[:, 0] < size) &
            (verts_proj[:, 1] >= 0) & (verts_proj[:, 1] < size)
        )
        ...
        face_vis_mask = visible_faces_mask(verts_proj, verts_cam, mesh.faces, size)
...
def load_reference_image(vid_name, ref_frame, size):
    """Load and horizontally flip the reference frame image."""
    ref_img_path = f'{PREPROCESSED_DATA}/{vid_name}/cropped/{ref_frame:05d}.jpg'
    ref_img = Image.open(ref_img_path).resize((size, size))
    return ref_img


def save_face_mask_image(out_dir, mesh, faces_idx, verts_proj, size, fname):
    ...
    for tri in mesh.faces[faces_idx]:
        rr, cc = polygon(verts_proj[tri, 1], verts_proj[tri, 0], (size, size))
        mask_img[rr, cc] = 255
    plt.imsave(os.path.join(out_dir, fname), mask_img, cmap='gray')


def compute_dense_flow(ref_faces, src_faces, ref_proj, src_proj, size):
    ...
    ref_proj_flipped[:, 0] = size - 1 - ref_proj_flipped[:, 0]
    src_proj_flipped[:, 0] = size - 1 - src_proj_flipped[:, 0]

    for face_ref, face_src in zip(ref_faces, src_faces):
        ref_tri = ref_proj_flipped[face_ref]
        src_tri = src_proj_flipped[face_src]
        rr, cc = polygon(ref_tri[:, 1], ref_tri[:, 0], (size, size))
        assert len(rr) > 0
        ...
        dense_flow[rr, cc, 0] = (src_pix[:, 0] / (size - 1)) * 2 - 1
        dense_flow[rr, cc, 1] = (src_pix[:, 1] / (size - 1)) * 2 - 1
    return dense_flow


def warp_image(src_img_path, dense_flow, size):
    """Warp source image using given dense flow."""
    img_target = Image.open(src_img_path).resize((size, size))
...
def main(vid_name: str):
    ...
    vis_data = calculate_visible_faces(ckpt_path, mesh_path, IMAGE_SIZE)

    ref_data = vis_data[ref_frame]
    ref_img = load_reference_image(vid_name, ref_frame, IMAGE_SIZE)

    for src_idx, data in vis_data.items():

        common_triangles = np.where(ref_data['face_mask'] & data['face_mask'])[0]
        ...
        ref_faces = ref_data['face_mesh'].faces[common_triangles]
        src_faces = data['face_mesh'].faces[common_triangles]
        dense_flow = compute_dense_flow(ref_faces, src_faces, ref_data['verts_proj'], data['verts_proj'], IMAGE_SIZE)

        src_img_path = f'{PREPROCESSED_DATA}/{vid_name}/cropped/{src_idx:05d}.jpg'
        warped_img, img_target = warp_image(src_img_path, dense_flow, IMAGE_SIZE)

        fig, axes = plt.subplots(1, 3, figsize=(12, 4))
        for ax, img, title in zip(axes, [ref_img, warped_img, img_target],["Frame 12 (ref)", f"Frame {src_idx} warped", f"Frame {src_idx}"]):
            ax.imshow(img)
            ax.set_title(title)
            ax.axis('off')
        plt.tight_layout()
